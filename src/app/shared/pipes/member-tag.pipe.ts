...
import { memberTagWithEmojiRegex } from '../regex';
import { memberTagWithEmojiRegexProfile } from '../regexProfile';
...
export class MemberTagPipe implements PipeTransform {
    ...
    transform(value: string, type = 'message', chatlist: boolean = false, searchListTrim: boolean = false, searchListKey: string = '') {
        ...
        if (memberTags) {
            var tokens = value.split(new RegExp(memberTags.join('|'), 'g'));
        }
        ...
        if (memberTags) {
            let counter = 0;
            value = value
                .replace(regEx1, '!tagged_member!')
                // .replace(regEx2, (i) => {
                //     return '&#' + i.charCodeAt(0) + ';';
                // })
                .replace(/!tagged_member!/g, (_) => {
