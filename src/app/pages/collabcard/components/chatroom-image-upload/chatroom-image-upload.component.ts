import { Component, ElementRef, EventEmitter, HostListener, Input, OnInit, Output, ViewChild, AfterViewInit, ChangeDetectorRef, ViewRef } from '@angular/core';
...
@Component({
  selector: 'app-chatroom-image-upload',
  templateUrl: './chatroom-image-upload.component.html',
  styleUrls: ['./chatroom-image-upload.component.scss']
})
export class ChatroomImageUploadComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.chatService.mediaPreviewOpened$$.next(true);
    setTimeout(()=>{
  ...
  generateAudioBlobUrl(){
    ...
    this.audioUrlMobile = this.audioUrl;
  ...
  playAudioFile(){
    this.audioObj.src = this.audioUrlMobile;    
    this.audioObj.load();
    this.audioObj.play();
  }
  ...
  closeDialog(): void {    
    ...
    this.chatService.mediaPreviewOpened$$.next(false);
  }
  ...
  uploadImage(event): void {
    ...
    for (const file of event.target.files) {
      ...
      reader.onload = e => {
        if (!this.files.find(img => img.name === file.name)) {
          this.files.push({
            file,
            blob: reader.result
          });
  ...
  uploadAudio(event): void {
    ...
    this.utilsService.fetchAllAudioFilesDuration(event.target.files).then((fileObjects) => {
        //console.log('THIS IS AUDIO FILE OBJECTS : ', fileObjects);
